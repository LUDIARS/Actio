...
// ─── project_id (EducationLab×Calliope PM 連携。2026-07-17 neco 最終裁定) ──
describe("tasks.project_id (EducationLab project scope)", () => {
  it("accepts projectId on create and returns it", async () => {
    const { status, json } = await request(app, "POST", "/api/tasks", {
      token,
      body: { title: "EducationLab task", projectId: "EducationLab-proj-1" },
    });
    expect(status).toBe(201);
    expect(json.task.projectId).toBe("EducationLab-proj-1");
  });

  it("accepts snake_case project_id alias on create", async () => {
    const { status, json } = await request(app, "POST", "/api/tasks", {
      token,
      body: { title: "EducationLab task snake", project_id: "EducationLab-proj-snake" },
    });
    expect(status).toBe(201);
    expect(json.task.projectId).toBe("EducationLab-proj-snake");
  });
