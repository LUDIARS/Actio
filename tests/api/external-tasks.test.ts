/**
 * 外部API tasks スコープ (EducationLab×Calliope PM 連携)
 *
 * project_id (EducationLab glab_project.id 等の不透明参照) 指定タスクの
 * read/write を api_clients トークンで行えることを検証する。
 * 2026-07-17 neco 最終裁定 / spec/tasks/2026-07-17-01-EducationLab-project-tasks.md
 */
...
const TASKS_CLIENT_ID = "client-EducationLab-tasks";
const OTHER_CLIENT_ID = "client-EducationLab-calendar-only";
...
beforeEach(async () => {
  clearTestDatabase();
  insertTestUser({ id: "svc-EducationLab", name: "EducationLab Service", email: "EducationLab-svc@test.com" });
...
describe("POST /api/external/tasks", () => {
  it("creates a task scoped to a project", async () => {
    const { status, json } = await request(app, "POST", "/api/external/tasks", {
      headers: tasksAuthHeaders(),
      body: { projectId: "proj-1", title: "New EducationLab task", ownerId: "student-1", estimatedMinutes: 45 },
    });
    expect(status).toBe(201);
    expect(json.task.projectId).toBe("proj-1");
    expect(json.task.ownerId).toBe("student-1");
    expect(json.task.title).toBe("New EducationLab task");
...
describe("GET /api/external/tasks/:id", () => {
  ...
  it("404s for tasks that have no project_id (personal tasks are out of scope)", async () => {
    ...
    const create = await request(app, "POST", "/api/tasks", {
      token,
      body: { title: "personal task, not EducationLab" },
    });
    const id = create.json.task.id;
