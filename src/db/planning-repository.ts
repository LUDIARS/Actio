...
export function planningRepositories() {
  if (dialect === "postgres") {
    ...
    return { backlog: new PostgresBacklogStore(client), sprints: new PostgresSprintStore(client), specs: new PostgresSpecImportStore(client) };
  }
  if (dialect !== "sqlite") throw new Error("Planning requires PostgreSQL or SQLite");
  const client = db.$client as SqliteDatabase;
  return { backlog: new BacklogStore(client), sprints: new SprintStore(client), specs: new SpecImportStore(client) };
}
