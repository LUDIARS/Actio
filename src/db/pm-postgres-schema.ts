...
import { pgTable, text, integer, doublePrecision, index, jsonb, timestamp } from "drizzle-orm/pg-core";
...
// ─── PM Task Snapshots ────────────────────────────────────
export const pmTaskSnapshots = pgTable(
  "pm_task_snapshots",
  {
    id: text("id").primaryKey(),
...
// ─── PM Task Validations ──────────────────────────────────
export const pmTaskValidations = pgTable(
  "pm_task_validations",
  {
    id: text("id").primaryKey(),
    taskId: text("task_id").notNull(),
    ...
    relatedCommits: jsonb("related_commits")
      ...
      .default([]),
    /** JSON: 対応テストファイル */
    testFiles: jsonb("test_files").$type<string[]>().notNull().default([]),
...
// ─── PM Conflicts ─────────────────────────────────────────
export const pmConflicts = pgTable(
  "pm_conflicts",
  {
    id: text("id").primaryKey(),
    taskId: text("task_id").notNull(),
...
// ─── PM Analytics Cache ───────────────────────────────────
export const pmAnalyticsCache = pgTable(
  "pm_analytics_cache",
  {
    id: text("id").primaryKey(),
    projectId: text("project_id").notNull(),
