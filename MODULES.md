...
|------|-----|
| ID | `placement` |
| ディレクトリ | `modules/placement/` |
| API パス | `/api/placement` |
| カテゴリ | ユーティリティ |

GPS 座標で「場所」 (place) を登録し、 端末の位置がその place の半径に入ったとき (`enter`) / 離れたとき (`leave`) に hook を発火する。 hook の action は現在 `webhook` のみ (任意 URL に POST)。

位置情報の入力経路:

- **OwnTracks (iOS/Android) → Imperativus (MQTT)** → POST `/api/placement/locations` (service-key 認証)
- 直接 POST もテスト時には可能 (\`X-Placement-Service-Key\` ヘッダ + body \`{user_id, lat, lon, ts?}\`)

主要 API:

| Method | Path | 用途 |
|---|---|---|
| POST | `/api/placement/locations` | Imperativus → Actio (位置情報 ingestion、 service-key auth) |
| GET | `/api/placement/places` | 自分の places 一覧 |
| POST | `/api/placement/places` | place 追加 (lat/lon 省略で現在地から) |
| DELETE | `/api/placement/places/:id` | place 削除 |
| GET | `/api/placement/current` | 現在地 + 今いる place |
| GET | `/api/placement/places/:id/hooks` | hook 一覧 |
| POST | `/api/placement/places/:id/hooks` | hook 追加 |
| PATCH | `/api/placement/places/:id/hooks/:hookId` | hook 更新 |
| DELETE | `/api/placement/places/:id/hooks/:hookId` | hook 削除 |

DB テーブル: `places` / `place_visits` / `place_hooks` / `placement_state`。 個人データ非保管 (id のみ FK、 lat/lon/name は Actio ドメイン固有なので保持)。

---

## モジュール依存関係

Actio はコアの **Event / Task** を中心に、各種モジュールがプラグインとして連携する。
