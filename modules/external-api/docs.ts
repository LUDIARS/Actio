...
export const apiDocumentation = {
  ...
  authentication: {
    ...
    scopes: {
      ...
      tasks: "project_id 指定タスクの読み取り・作成・状態/優先度/見積り更新 (EducationLab×Calliope PM 連携用。既定スコープには含まれず opt-in)",
    },
    keyManagement: {
  ...
  modules: {
    tasks: {
      description:
        "project_id (EducationLab glab_project.id 等の不透明参照) 指定タスクの操作 (scope: tasks)。" +
        " EducationLab×Calliope PM 連携用 (2026-07-17 neco 最終裁定)。 project は必須パラメータ。",
      ...
      endpoints: [
        ...
        },
        {
          ...
          body: {
            projectId: "string (必須, EducationLab glab_project.id 等)",
            title: "string (必須)",
