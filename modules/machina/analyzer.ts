...
import type { MachinaTaskPriority } from "../../src/shared/constants.js";
import {
  MACHINA_COMPLETION_KEYWORDS,
  MACHINA_URGENCY_KEYWORDS,
...
export function analyzeMessage(input: AnalysisInput): AnalysisResult {
  ...
  const isCompletion = MACHINA_COMPLETION_KEYWORDS.some((kw) =>
 ...
 * テキストから優先度を判定
 */
function determinePriority(text: string): MachinaTaskPriority {
  const hasUrgency = MACHINA_URGENCY_KEYWORDS.some((kw) => text.includes(kw));
