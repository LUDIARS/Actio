...
| `tasks` | `/api/external/tasks/*` | `project_id` (EducationLab `glab_project.id` 等の不透明参照) 指定タスクの read/write (create / update status・priority・estimatedMinutes)。EducationLab×Calliope PM 連携 (2026-07-17 neco 最終裁定)。project_id が設定されていないタスクにはアクセス不可。**opt-in スコープ** (POST `/api/external/clients` で `scopes` 省略時の既定セットには含まれない) |
